if FileTest.exist?("config/rubber/rubber-percona.yml")
  ...
  "db_backup_cmd: \"\#\{mount_directory\}/db-backup-tools/rubber-xtrabackup.sh -u %user% -p %pass% -t \#\{mount_directory\}/db_backups -b %backup_file%\"\n"
  gsub_file "config/rubber/rubber-percona.yml", /^db_restore_cmd.*$/, \
  "# Replaced by xtrabackup vulcanizer.\n# \\0\n" + \
  "db_restore_cmd: \"\#\{mount_directory\}/db-backup-tools/rubber-xtrarestore.sh\"\n"
